...
const prettifyMs = require("pretty-ms");
...
const DropletWateringIndicator = styled.div`
  display: flex;
    ...
    <Info>
      <DropletWateringIndicator>
        ...
        <div style={{ width: "90%", marginLeft: "10px" }}>
          {status.isWatering ? "Watering In Progress" : "Not Watering"}
        </div>
      </DropletWateringIndicator>
      ...
      <div>
        ...
        {status.timeRemaining > 0 && (
          <p>
            Time Remaining :
            {prettifyMs(status.timeRemaining, { compact: true })} seconds
          </p>
        )}
      </div>
    </Info>
